...
import (
	...
	"github.com/404nffff/go_pkg/yml_config"
)
...
func loadConfig(conn string) DatabaseConfig {
	...
	config := DatabaseConfig{
		...
		SlowThreshold:      mysqlConfig.GetInt(conn + ".SlowThreshold"),
		EventDestroyPrefix: config.EventDestroyPrefix + "Mysql_" + conn,
		LogLevel:           mysqlConfig.GetString(conn + ".LogLevel"),
	}
