...
func ZapInit(entry func(zapcore.Entry) error) *zap.Logger {

	// 获取程序所处的模式：  开发调试 、 生产
	//config.ConfigYml := yml_config.CreateYamlFactory()
	appDebug := config.ConfigYml.GetBool("AppDebug")
	...
	switch config.ConfigYml.GetString("Logs.TextFormat") {
	case "console":
	...
	//写入器
	fileName := config.BasePath + config.ConfigYml.GetString("Logs.GoSkeletonLogName")
	lumberJackLogger := &lumberjack.Logger{
		Filename:   fileName,                                     //日志文件的位置
		MaxSize:    config.ConfigYml.GetInt("Logs.MaxSize"),    //在进行切割之前，日志文件的最大大小（以MB为单位）
		MaxBackups: config.ConfigYml.GetInt("Logs.MaxBackups"), //保留旧文件的最大个数
		MaxAge:     config.ConfigYml.GetInt("Logs.MaxAge"),     //保留旧文件的最大天数
		Compress:   config.ConfigYml.GetBool("Logs.Compress"),  //是否压缩/归档旧文件
	}
